...
angular.module('yaru22.angular-timeago', [
	...
	service.inWords = function (distanceMillis) {
		...
		var ago = [prefix, words, suffix].join(separator).trim();

		var timestamp = new Date().getTime() - distanceMillis;

		return date_filter(timestamp) + ' (' + ago + ')';
	};

	service.parse = function (iso8601) {
		return parseInt(iso8601, 10) * 1000;
	};
